...
if ($_FILES["ayar_logo"]) {
  ...
  $yuklemeYeri = __DIR__ . DIRECTORY_SEPARATOR . $yol . DIRECTORY_SEPARATOR . $_FILES["ayar_logo"]["name"];
  ...
  if ( file_exists($yuklemeYeri) ) {
      ...
      if ($_FILES["ayar_logo"]["size"]  > 1000000) {
          ...
          $dosyaUzantisi = pathinfo($_FILES["ayar_logo"]["name"], PATHINFO_EXTENSION);
          ...
          if ($dosyaUzantisi != "jpg" && $dosyaUzantisi != "png") { # Dosya uzantı kontrolü
              ...
              $sonuc = move_uploaded_file($_FILES["ayar_logo"]["tmp_name"], $yuklemeYeri);
...
if (isset($_POST['alankaydet'])) {
	...
	$kaydet=$db->prepare("INSERT INTO alan SET
		alan_ad=:ad,
		alan_icon=:icon,
		alan_kisa=:kisa,
		alan_detay=:detay");
	$insert=$kaydet->execute(array(
		'ad' => $_POST['alan_ad'],
		'icon' => $_POST['alan_icon'],
		'kisa' => $_POST['alan_kisa'],
		'detay' => $_POST['alan_detay']
		));

	if ($insert) {

		Header("Location:../production/alan.php?durum=ok");

	} else {

		Header("Location:../production/alan.php?durum=no");
	}

}


if (isset($_POST['alanduzenle'])) {



		$duzenle=$db->prepare("UPDATE alan SET
		alan_ad=:ad,
		alan_icon=:icon,
		alan_kisa=:kisa,
		alan_detay=:detay
		...
		if ($update) {
			...
			Header("Location:../production/alan-duzenle.php?alan_id=$alan_id&durum=ok");

		} else {

			Header("Location:../production/alan-duzenle.php?durum=no");
		}
...
if ($_GET['alansil']=="ok") {
	...
	$sil=$db->prepare("DELETE from alan where alan_id=:alan_id");
	$kontrol=$sil->execute(array(
